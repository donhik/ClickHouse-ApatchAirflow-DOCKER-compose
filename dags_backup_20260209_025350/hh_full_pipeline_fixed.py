...
def hh_full_pipeline_fixed():
    """Полный пайплайн сбора и сохранения данных"""
    print("=" * 60)
    print("🚀 ПОЛНЫЙ ПАЙПЛАЙН HH.RU -> CLICKHOUSE (ИСПРАВЛЕННЫЙ)")
    print("=" * 60)

    try:
        # 1. Получаем данные с hh.ru (оставляем как есть)
        ...
        # 2. Сохраняем в ClickHouse (ИСПРАВЛЕННАЯ ЧАСТЬ!)
        ...
        print(f"SQL запрос подготовлен для {len(all_vacancies)} вакансий")

        # ИСПРАВЛЕНИЕ: Используем POST запрос вместо GET
        ...
        # Получаем статистику (GET запросы для чтения работают)
        stats_queries = {
        ...
        # Сохраняем отчет
        with open(f"/tmp/hh_report_fixed.txt", "w", encoding="utf-8") as f:
            f.write("\n".join(report))

        print("💾 Отчет сохранен в /tmp/hh_report_fixed.txt")
...
with DAG(
    'hh_full_pipeline_fixed',
    default_args=default_args,
    description='Полный пайплайн сбора вакансий (исправленный)',
    ...
    main_task = PythonOperator(
        task_id='run_full_pipeline_fixed',
        python_callable=hh_full_pipeline_fixed
    )
